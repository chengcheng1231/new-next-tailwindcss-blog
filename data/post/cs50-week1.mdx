...
category: 'coding'
author: Chengcheng
imgSrc: '/static/images/cs50-week0/p5p1.png'
tags: ['cs50']
...
```
但如果今天我們想印出一百次 `meow` ，我們一開始會想複製貼上 `printf`，但太麻煩也不準確，因此程式中有 loop 也就是迴圈的概念來幫助我們達到此效果，如以下以 `while loop` 的方式來寫，`i` 代表的是一個 counter，當 `i` 小於 3 則執行 `{}` 內的程式，最後記得要 `i++` 來讓 counter 加一。
